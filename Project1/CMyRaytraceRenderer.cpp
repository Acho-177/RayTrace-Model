...
void CMyRaytraceRenderer::RayTexture(CGrTexture* texture, CGrPoint texcoord, double* raytexture)
{
	...
	if (texture != NULL && !texture->Empty())
	{
		...
		// Calculate the indices of the four nearest pixels surrounding the given UV coordinate
		int x0 = U / 1;
		int x1 = x0 + 1;
		int y0 = V / 1;
		int y1 = y0 + 1;
		// Ensure that the indices are within the bounds of the texture image
		if (x1 >= texture->Width()) x1 = texture->Width() - 1;
		if (y1 >= texture->Height()) y1 = texture->Height() - 1;

		double x = texcoord[0] * texture->Width();  // Calculate the x index of the pixel
		double y = texcoord[1] * texture->Height(); // Calculate the y index of the pixel
