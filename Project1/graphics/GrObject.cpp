...
{
    CGrPtr<CGrPolygon> poly;
    double angle_step = 2 * 3.1415926 / num_segments;

    // Create front polygons
    ...
    for (int i = 0; i < num_segments; i++) {
        ...
        double x_pos2 = x + radius * cos(angle + angle_step);
        double y_pos2 = y + radius * sin(angle + angle_step);

        poly = new CGrPolygon;
        poly->AddNormal3d(cos(angle + angle_step / 2), sin(angle + angle_step / 2), 0);
